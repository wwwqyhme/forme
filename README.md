...
| onErrorChanged | false | `FormeErrorChanged` | triggered when field's validation error changed |
...
## Custom way display error text
 ...
 you can display custom error via `FormeField`'s `onErrorChanged` or `FormeFieldController`'s  `errorTextListenable` if default error text display can not fit your needs.

**don't forget to set Forme's quieltyValidate attribute to true**

### via onErrorChanged

`onErrorChanged` will triggered when field's errorText changed , it is suitable when you want to update a field according to error state of field 
